...
int main()
{
  ...
  do
  {
    ...
    status = mcp9808_sensor->set_mod_of_operation(MCP9808::MODE_OF_OPERATION::CONTINUOUS_CONVERSION);	
    if (status) {
      std::cout << "Setting mod of operation failed with error " << status << std::endl;
      break;
    }
  
    status = mcp9808_sensor->set_resolution(MCP9808::RESOLUTION::RES_0_25_DEG);
    if (status) {
      std::cout << "Setting the resolution for the tenperature measurement with error " << status << std::endl;
      break;
    }
    ...
    if (status) {
      std::cout << "Reading the temperature from the sensor failed with error " << status << std::endl;
      break; 
    }
