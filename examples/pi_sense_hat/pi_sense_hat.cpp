...
int main() {
  ...
  LPS25H *lps25h_sensor =
      new LPS25H(Slave_Device_Type::I2C_SLAVE_DEVICE, i2c_bus_master, 0x5C);
  HTS221 *hts221_sensor =
      new HTS221(Slave_Device_Type::I2C_SLAVE_DEVICE, i2c_bus_master, 0x5F);
  ...
  // Get the humidity and temperature readings from the sensor and calculate the
  ...
  std::cout << std::endl
            << "Readings from Pi Sense Hat sensors:" << std::endl
            << std::endl;
