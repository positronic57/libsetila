...
void I2C_Slave_Device::dettach_from_master_bus() { m_bus_master = nullptr; }

int I2C_Slave_Device::read(uint8_t address, void *buffer, int buffer_size) {
  return I2C::i2c_slave_read(m_bus_master->bus_master_fd(), m_I2C_slave_address,
                             address, buffer, buffer_size);
}

int I2C_Slave_Device::write(uint8_t address, void *buffer, int buffer_size) {
  return I2C::i2c_slave_write_data(m_bus_master->bus_master_fd(),
                                   m_I2C_slave_address, address, buffer,
                                   buffer_size);
}

int I2C_Slave_Device::write_byte(uint8_t address, uint8_t value) {
  return I2C::i2c_slave_write_byte(m_bus_master->bus_master_fd(),
                                   m_I2C_slave_address, address, value);
}

int I2C_Slave_Device::read_write(void *transmit_buffer, void *receive_buffer,
                                 int buffer_size) {
  return ERROR_IO_MODE_NOT_SUPPORTED; // I2C does not support full-duplex
                                      // communication.
}
