...
int SRF02::srf02_find_object(SRF02_Command measurement_unit) {
   ...
   * register. */
  if (this->write_byte(static_cast<uint8_t>(SRF02_Register::COMMAND),
   ...
   */
  if (this->read(static_cast<uint8_t>(SRF02_Register::COMMAND), receive_buffer,
                 4)) {
    return ERROR_WRITE_FAILED;
  }

  m_software_version =
      receive_buffer[static_cast<int>(SRF02_Register::COMMAND)];

  /* Build the range value from the two range registers. */
  m_distance =
      receive_buffer[static_cast<int>(SRF02_Register::RANGE_HIGH_BYTE)];
  m_distance <<= 8;
  m_distance +=
      receive_buffer[static_cast<int>(SRF02_Register::RANGE_LOW_BYTE)];
