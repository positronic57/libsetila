...
int SPI_Bus_Master_Device::configure(SPI_BUS_MODE spi_bus_mode,
                                     uint32_t spi_bus_max_speed_hz,
  ...
  configure_status =
      ioctl(bus_master_fd(), SPI_IOC_WR_BITS_PER_WORD, &spi_bits_per_word);
  if (configure_status < 0) {
    return ERROR_CONF_BUS_MASTER_FAILED;
  }

  configure_status =
      ioctl(bus_master_fd(), SPI_IOC_RD_BITS_PER_WORD, &spi_bits_per_word);
  if (configure_status < 0) {
    return ERROR_CONF_BUS_MASTER_FAILED;
  }
  ...
  configure_status =
      ioctl(bus_master_fd(), SPI_IOC_WR_MAX_SPEED_HZ, &spi_bus_max_speed_hz);
  if (configure_status < 0) {
    return ERROR_CONF_BUS_MASTER_FAILED;
  }

  configure_status =
      ioctl(bus_master_fd(), SPI_IOC_RD_MAX_SPEED_HZ, &spi_bus_max_speed_hz);
  if (configure_status < 0) {
    return ERROR_CONF_BUS_MASTER_FAILED;
  }
