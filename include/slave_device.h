...
enum class Slave_Device_Type : int {
  GENERIC_SLAVE_DEVICE =
      0, /**< A generic device type. A dummy value, not used in practice. */
  I2C_SLAVE_DEVICE, /**< A slave device connected to I2C bus. */
...
class Slave_Device {
private:
  Slave_Device_Type m_slave_device_type =
      Slave_Device_Type::GENERIC_SLAVE_DEVICE; /**< Defines the slave device
                                                  type based on the underlying
                                                  bus. */
...
public:
   ...
   *
   * @param[in] slave_device_type defines the slave device type based on the
   * underlying bus
   */
   ...
   */
  const Slave_Device_Type slace_device_type() const {
    return m_slave_device_type;
  };

  /**
   * @brief Assigns a bus muster device to the slave device.
   * A virtual function, implemented in the child class(es) based on the slave
   * device type.
   *
   * @param bus_master_device a pointer to the object that represents the bus
   * master device
   * @return 0 for a non null bus master device pointer and a correct bus master
   * type, -1 otherwise
   */
   ...
   *
   * @param[in] address the data will be read from the slave device starting
   * form this internal address/register
   * @param[out] buffer input buffer (contains the data read from the slave
   * device)
   * @param[in] buffer_size size of the input buffer (maximal number of bites
   * that will be read from the device)
   *
   * @return 0 in case all required IO operations on the bus are successful, -1
   * otherwise
   */
   ...
   *
   * @param[in] address data will be written into the slave device starting from
   * this internal address/register
   * @param[in] buffer output buffer (contains the data sent to the slave device
   * for writing)
   ...
   * Each slave implements it's own write method based on the underlying bus.
   * Useful for changing the value of a specific internal register of the slave
   * device.
   *
  ...
  /**
   * @brief A generic function for data exchange between the slave and the
   * master device in case of a full-duplex synchronous bus like SPI.
   *
   * @param[in] transmit_buffer content of this buffer will be sent to the slave
   * device
   * @param[out] receive_buffer the response of the slave device will be written
   * here
