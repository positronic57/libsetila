...
private:
  ...
  float m_reference_voltage =
      0.0; /**< Reference voltage value. Required for representing the digital
              reading as analog value. */
...
public:
  ...
  explicit MCP3204(float reference_voltage)
      : SPI_Slave_Device(), m_digital_value(0),
  ...
  /**
   * @brief Calculate the value of the analog input equivalent to the digital
   * reading from MCP3204.
   *
   * @return analog equivalent to the digital reading
   */
  float analog_value() {
    return (m_digital_value * m_reference_voltage) / 4096;
  };
};
