 ...
 *
 * @brief Simple moving average filter implementation using circular buffer as a
 * moving window.
 *
 * Can be used on real-time data (readings from a sensor) or applied on a vector
 * of already sampled data.
 *
 ...
 */
template <class tElement> class Moving_Average_Filter {
private:
  tElement m_sum;         /**< Sum of all elements in the moving window */
  tElement m_last_output; /**< The last output of the filter */
  CircularBuffer<tElement> *m_buffer =
      nullptr; /**< Internal circular buffer representing the moving window */
...
public:
  ...
  ~Moving_Average_Filter() { delete m_buffer; }

  /**
   * @brief Calculates on the fly the output of the filter as a mean value for
   * the elements in the window.
   *
   * During the initial filling of the circular buffer the sampling window size
   * is equal to the data-set size. When there are enough samples for filling
   * the circular buffer, the full window size is used for calculating the
   * average value of the samples in the window.
   *
   ...
   */
  void filter(const std::vector<tElement> *input,
   ...
   * @return
   */
  const tElement last_output() const { return m_last_output; }
};
