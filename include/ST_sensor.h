 ...
 *
 * The data communication interface is defined as an instance of the
 * Slave_Device class and can cover both I2C and SPI.
 *
...
class ST_Sensor {
private:
  Slave_Device_Type m_interface_type =
...
public:
   ...
   * support both of them. The constructor defines which interface type will be
   * used by the sensor for data exchange. As slave device, the sensor must be
   * attached to a bus controlled by the master device. When connected to I2C
   * bus, the sensor requires I2C address.
   *
   * @param[in] interface_type Slave_Device_Type selects the data interface type
   * @param[in] bus_master_device pointer to the instance of the
   * Bus_Master_Device class
   * @param[in] i2c_address I2C address in case of I2C interface, ignored for
   * SPI and set to default value of 0x03 if not specified
   */
  explicit ST_Sensor(Slave_Device_Type interface_type,
                     Bus_Master_Device *bus_master_device,
                     uint8_t i2c_address = 0x03);
   ...
   *
   * @param[in] who_am_I_reg_addr address of the WHO_AM_I register of the ST
   * Sensor
   * @param[in] id_value expected value of the WHO_AM_I register for the sensor
   * @return int 0 if the verification was successful, ERROR_WRONG_DEVICE_MODEL
   * in case the value of the WHO_AM_I register does not match,
   * ERROR_READ_FAILED for failed attempt to read the WHO_AM_I register
   */
  int verify_device_id(uint8_t who_am_I_reg_addr, uint8_t id_value);

  /**
   * @brief High level function for configuring the sensor. It sets the mode of
   * operation and the output data rate by writing values in the sensors control
   * registers based on the given arguments. The registers and their values for
   * achieving the desired sensor configuration are individual for each ST
   * sensor type. The logic will be implemented in the child class that
   * represents the target ST sensor.
   *
   * @param[in] mode_of_operation predefined enum for the modes of operation
   * found in ST sensors
   * @param[in] output_data_rate predefined enum for the output data data rates
   * found in ST sensors
   *
   * @return int 0 in case of successful configuration write, appropriate error
   * code otherwise
   */
  virtual int
  set_mode_of_operation(MODE_OF_OPERATION mode_of_operation,
                        OUTPUT_DATA_RATE output_data_rate = ODR_ONE_SHOT) = 0;
  ...
  virtual int set_mode_of_operation(
      ...
      FIFO_TYPE fifo_type = ST_Sensor::FIFO_TYPE::FIFO_DISABLED) = 0;

  /**
   * @brief Set the resolution for the physical quantity(ies) measured by the
   * sensor. Different ST sensors support different measurement resolution(s).
   * The registers used for that purpose varies from sensor to sensor. The
   * functionality must be implemented in the child class following the correct
   * way for the targeted ST sensor.
   *
   * @param[in] average_1 bit coded number of averaged samples for the first
   * physical quantity
   * @param[in] average_2 bit coded number of averaged samples for the second
   * physical quantity if any
   *
  ...
  /**
   * @brief High level function for reading the values of the data registers for
   * the measured physical quantities. Based on the defined mode of operation,
   * the actual data reading can be preceded with a command for measurement
   * start. As other two functions, this procedure is also a sensor dependent.
   *
   * @return int 0 for success, error code otherwise
   */
