 ...
 * @file srf02.h
 *
 * @brief Contains the definition of SRF02 class for communication with SRF02
 * sonar sensor.
 *
...
public:
  static constexpr uint8_t RANGING_IN_PROGRESS{0xFF};
  ...
  enum class Status : int8_t {
    READ_WRITE_ERROR = -5,     //!< SRF02_READ_WRITE_ERROR
    UNSUPPORTED_COMMAND = -4,  //!< SRF02_UNSUPPORTED_COMMAND
    OBJECT_FOUND = -3,         //!< SRF02_OBJECT_FOUND
    NO_OBJECT_IN_RANGE = -2,   //!< SRF02_NO_OBJECT_IN_RANGE
    READING_RANGE_FAILED = -1, //!< SRF02_READING_RANGE_FAILED
    OPERATION_SUCCESSFUL = 0   //!< SRF02_OPERATION_SUCCESSFUL
  };

private:
  uint8_t m_software_version =
      0x00; /**< Holds the firmware version of the sensor read from the internal
               sensor registers. */
  uint16_t m_distance =
      0x00; /**< Holds the distance value measured by the sensor. */

public:
  SRF02() : I2C_Slave_Device(SRF02::I2C_Address) {};
   ...
   *
   * @return request status of type srf02_status_t. SRF02_OBJECT_FOUND in case
   * of a successful data exchange with SRF02, status code otherwise
   */
  SRF02::Status srf02_find_object(SRF02::Command measurement_unit);

  SRF02::Status srf02_find_object(SRF02::Command measurement_unit,
                                  uint16_t *measured_distance);
  /**
   * @brief Implements the procedure for changing the I2C address of the SRF02
   * module as described in the data sheet.
   *
   * @param[in] new_i2c_address the new I2C address of type uint8_t
   * @return status of the request of type srf02_status_t.
   * SRF02_OPERATION_SUCCESSFUL when the communication with the I2C was
   * succsefful, a status code otherwise
   */
  SRF02::Status srf02_set_new_i2c_address(uint8_t new_i2c_address);
   ...
   */
  std::string_view srf02_status_to_string(SRF02::Status srf02_status);
};
