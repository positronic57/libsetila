...
private:
  BUS_TYPE m_bus_type =
      BUS_TYPE::DUMMY; /**< Defines the bus type: SPI or I2C. */
  int m_bus_master_fd =
      -1; /**< File descriptor for data exchange with the bus master driver. */
  std::string m_bus_master_device_file_name =
      "/dev/null"; /**< Name of the Linux device file created by the bus master
                      driver. */
...
public:
  ...
  /**
   * @brief Constructor.
   * @param[in] bus_master_device_file_name name of the Linux device file
   * created by the master driver
   * @param[in] bus_type	bus type managed by the master driver
   */
  explicit Bus_Master_Device(const std::string &bus_master_device_file_name,
                             BUS_TYPE bus_type);
   ...
   * @return int 0 in case of a successful file device open, ERROR_OPEN_BUS
   * otherwise.
   */
  int open_bus(void);
  ...
  /**
   * @brief Provides the values of the file descriptor for accessing the bus
   * driver to the requesters(slave devices).
   * @return value of the m_bus_master_fd class member
   */
  int bus_master_fd() const { return m_bus_master_fd; };

  /**
   * @brief Gives the number of the slave devices that requested the file
   * descriptor for accessing the bus master driver.
   *
