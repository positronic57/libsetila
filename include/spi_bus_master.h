...
/**
 * \enum SPI_BUS_MODE
 * @brief Defines the SPI modes (combinations of polarity and phases of the bus
 * signals).
 */
...
class SPI_Bus_Master_Device : public Bus_Master_Device {
  uint32_t m_spi_bus_max_speed_hz =
      0; /**< Maximal SPI bus speed in Hz. Value dictated by the SPI slave. */
  SPI_BUS_MODE m_spi_bus_mode =
      SPI_BUS_MODE::MODE_0; /**< SPI mode (polarity and phase of the bus
                               signal). */
  uint16_t m_spi_delay =
      0; /**< how long to delay after the last bit transfer before optionally
            deselecting the device before the next transfer. As described in the
            description of the spi_ioc_transfer structure. */
  uint8_t m_spi_bits_per_word = 8; /**< Device's word size. Defined based on
                                      slave devices requirements. */

public:
  SPI_Bus_Master_Device()
      : Bus_Master_Device("/dev/spidev0.1", BUS_TYPE::SPI_BUS) {};
  /**
   * @brief Class constructor. Provides the values of the SPI bus properties and
   * the data required for parent class initialization.
   *
  ...
  /**
   * @brief Sets the parameters of the SPI bus. If not later overwritten by SPI
   * slaves init/configure function, these parameters will be used by the SPI
   * slave objects for data exchange on the bus.
   *
   * @param[in] spi_bus_mode  SPI mode (polarity and phase of the bus signal).
   * @param[in] spi_bus_max_speed_hz	Maximal SPI bus speed in Hz. Value
   * dictated by the SPI slave.
   * @param[in] spi_bits_per_word Device's word size. Defined based on slave
   * devices requirements.
   * @param[in] spi_delay	how long to delay after the last bit transfer
   * before optionally deselecting the device before the next transfer.
   * @return
   */
  int configure(SPI_BUS_MODE spi_bus_mode, uint32_t spi_bus_max_speed_hz,
                uint8_t spi_bits_per_word, uint16_t spi_delay);
